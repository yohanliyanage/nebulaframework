 ...
 * <p>
 * Spring JMS constructs are used to listen to available {@code GridTask}s in the
 * {@code TaskQueue}, then does the execution of the {@code GridTask} by calling
 * {@link GridTask#execute()} method, and finally writes the result of execution
 * to the {@code ResultQueue}, wrapped in a {@code GridTaskResult}.
 * <p>
 * Furthermore,  special Class Loading mechanisms are also utilized, according to
 * the type of {@code GridJob}, as follows:
 * <p>
 * <table border='1'>
 * 	<tr><td><b>Type</b></td><td><b>ClassLoader</b></td></tr>
 * 	<tr><td>Non-archived {@code GridJob}</td><td>{@link GridNodeClassLoader}</td></tr>
 * 	<tr><td>Archived {@code GridJob}</td><td>{@link GridArchiveClassLoader}</td></tr>
 * </table>
 * <p>
 * The relevant {@code ClassLoader} is attached to the thread of execution for the 
 * {@code GridTask} as the {@code contextClassLoader} so that it will be utilized 
 * for necessary Class Loading during the execution of Task.
 * <p>
 * Also, {@code TaskExecutor} keeps a reference of each active {@code TaskExecutor}
 * instance, against the {@code JobId} so that the allocated resources could be
 * released at the end of Job Execution.
 * 
