...
/*
 ...
 Customized by Yohan Liyanage on 04/08/2008 for using as a
 benchmark test for Nebula Framework.

 Original Version : http://www.netlib.org/benchmark/linpackjava/
 ------------------------------------------------------------
 ...
 * Linpack Benchmark.
 * <p>
 * This code was downloaded from http://www.netlib.org/benchmark/linpackjava/
 * and customized for Nebula Benchmark Process.
 */
@SuppressWarnings("unused")
public class Linpack {

	public static final int N_CONSTANT = 500;
	public static final int LDAA_CONSTANT = N_CONSTANT * 2;
	...
	public static void main(String[] args) {
		...
		//l.run_benchmark();
		System.out.println(l.runBenchmark().toString());
	...
	public LinPackResult runBenchmark(int cycles) {
		
		LinPackResult finalResult = new LinPackResult();
		
		finalResult.setCycles(0);
		
		for (int i=0; i < cycles; i++) {
			LinPackResult r = runBenchmark();
			finalResult.setCycles(finalResult.getCycles()+1);
		}
	 ...
	 * @return result
	 */
	public LinPackResult runBenchmark() {

		int n = N_CONSTANT;
		int ldaa = LDAA_CONSTANT;
	    ...
	    double eps_result = epslon((double)1.0);
		...
		result.setMflops(mflops_result);
		result.setNormRes(residn_result);
		result.setPrecision(eps_result);
		result.setTime(time_result);
		
		return result;
		
	}

	public static double getOps(int n) {
		return (2.0e0 * (n * n * n)) / 3.0 + 2.0 * (n * n);
	}
	
	public static double calculateMFLOPS(int taskCount, double timeSecs) {
		double mflops_result = (getOps(N_CONSTANT) / (1.0e6 * timeSecs)) * taskCount;
		mflops_result += 0.0005; // for rounding
		mflops_result = (int) (mflops_result * 1000);
		mflops_result /= 1000;
